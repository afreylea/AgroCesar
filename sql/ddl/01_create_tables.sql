...
CREATE TABLE CULTIVOS_CATALOGO (
  ...
  CONSTRAINT CK_CATALOGO_TEMP_RANGO CHECK (TEMP_MIN >= -10 AND TEMP_MAX <= 60),
  CONSTRAINT CK_CATALOGO_LLUVIA     CHECK (LLUVIA_MAX > 0),
  CONSTRAINT CK_CATALOGO_HUMEDAD    CHECK (HUMEDAD_MAX BETWEEN 0 AND 100),
...
CREATE TABLE ALERTAS (
  ...
  CONSTRAINT CK_ALERTAS_TIPO CHECK (TIPO_ALERTA IN (
    'TEMPERATURA_ALTA','TEMPERATURA_BAJA','LLUVIA_EXCESIVA','HUMEDAD_EXCESIVA'
  )),
